...
async function voirAvis() {
    ...
    try {
        const items = await fetchData("https://arcadia35380-f680d3a74682.herokuapp.com/api/avis/get", myHeaders);

        const avissContainer = document.getElementById("voirAvis");
        avissContainer.innerHTML = ''; // Vide le conteneur avant d'ajouter les nouveaux éléments
        ...
        items.forEach(item => {
            ...
            visibleElement.addEventListener('click', async () => {
                ...
                try {
                    ...
                    const response = await fetch(`https://arcadia35380-f680d3a74682.herokuapp.com/api/avis/${item.id}`, putRequestOptions);

                    // Vérification des erreurs de la réponse
                    if (!response.ok) {
                        const errorResponse = await response.json(); // Lire la réponse JSON
                        console.error(`Error: ${errorResponse.error}`); // Affiche l'erreur reçue
                        throw new Error(`Failed to update avis ${item.id}: ${errorResponse.error}`);
                    }
                    ...
                    visibleElement.textContent = newValue ? "Afficher" : "Cacher";
                    ...
                    alert(`An error occurred while updating visibility for avis ${item.id}.`);
                }
            });
            ...
            // Ajouter le rowElement (description + visibilité) à l'avis principal
            avisElement.appendChild(rowElement);

            // Ajouter l'élément d'avis au conteneur principal
...
async function voirHabitat() {
    ...
    try {
        const items = await fetchData("https://arcadia35380-f680d3a74682.herokuapp.com/api/habitat/get", myHeaders);
...
async function modifierHabitat(habitatId, oldImageData, nom, description, imageInput) {
    ...
    try {
        const response = await fetch(`https://arcadia35380-f680d3a74682.herokuapp.com/api/habitat/${habitatId}`, {
            method: 'PUT',
            headers: myHeaders,
...
async function voirAnimal() {
    ...
    try {
        const items = await fetchData("https://arcadia35380-f680d3a74682.herokuapp.com/api/animal/get", myHeaders);
...
async function updateAnimal(id, grammage, feeding_time, created_at) {
    ...
    try {
        const response = await fetch(`https://arcadia35380-f680d3a74682.herokuapp.com/api/animal/${id}`, {
            method: 'PUT',
            headers: myHeaders,
